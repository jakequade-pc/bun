...
struct us_socket_t *us_internal_socket_close_raw(struct us_socket_t *s, int code, void *reason) {
    /* No ssl_in_use deferral here (unlike us_internal_ssl_close): close_raw
     * is the force-close primitive the close_all force-drain relies on for
     * progress, so it always unlinks. The only ssl_in_use-sensitive step
     * below is SSL_free, and us_internal_ssl_detach defers that on its own. */
